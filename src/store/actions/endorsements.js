...
export default {
  ...
  async saveMovement({ state, commit }, payload) {
    try {
      ...
      if (!comparison[column] && typeof comparison[column] !== "boolean") return;
  ...
  async getEndorsementInformation({ commit, state }, table = "newInsurance") {
    try {
      ...
      const filter = Array.from(tableResponse).map((obj) => {
        if (obj["movementEndDate"]) obj["movementEndDate"] = new Date(obj["movementEndDate"]).toISOString().split("T")[0];
        if (obj["effectiveDateMovement"]) obj["effectiveDateMovement"] = new Date(obj["effectiveDateMovement"]).toISOString().split("T")[0];
        if (obj["premiumPaymentDate"]) obj["premiumPaymentDate"] = new Date(obj["premiumPaymentDate"]).toISOString().split("T")[0];
  ...
  async getEndorsements({ commit, state }) {
    try {
      ...
      if (statusCode !== 200) throw new Error("Error while fetching endorsements");
   ...
   */
  async createEndorsment({ commit, state }, { subscription_id, endorsment_type, damage, BI, Stocks, exprired_date, percentaje }) {
    try {
      const { data } = await apolloClient.mutate({
  ...
  async saveNewDocumentEndorsement({ state, commit, dispatch }, { document_id, subscription_id, type, docS3 }) {
    try {
      const {
  ...
  async savedocumentEndorsement({ commit, dispatch }, { document_id, subscription_id, type, docS3 }) {
    try {
      const {
        data: { saveDocument },
      } = await apolloClient.mutate({
