...
export default {
  async saveQuotationColumn({ commit, state }, { table = "quotation", parent, column }) {
    try {
  ...
  async getQuotationInformation({ commit, state }) {
    try {
      ...
      const parsedResponse = JSON.parse(findResponse.data["findQuotationBySubscription"].response);
      ...
      const filterFirst = Object.keys(QUOTATION.quotation)
        ...
        .reduce((obj, key) => {
          ...
          if (key === "inceptionDate" || key === "expiryDate") obj[key] = new Date(quotation[key]).toISOString().split("T")[0];
          return obj;
        }, {});

      /* special options */
      const item = state.currencies.find((item) => item.id === state.accountInformation.currency);
  ...
  async getToBeDefinedBySubscriptionId({ commit }, subscriptionId) {
    try {
      ...
      const parsedResponse = JSON.parse(findResponse.data["getToBeDefinedBySubscriptionId"].response);
      const responseToCamel = keysToCamel(parsedResponse);
  ...
  async createToBeDefined({ commit, state }, payload) {
    try {
      ...
      if (statusCode !== 200) throw new Error("Error while creating To Be Defined");
  ...
  async getQuotationColumnData({ commit, state }, { table = "quotation", column, subscriptionId = state.subscription_id }) {
    try {
      const variables = {
        table,
        column,
