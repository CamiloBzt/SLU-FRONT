          ...
          <div class="ExpandContent">
            ...
            <div class="ExpansionInputContent mb-7">
              <v-text-field
                ...
                required
                :error-messages="
                  requiredInputVuelidateParent('name', 'createContactData')
                "
                label="Contact Name"
              >
              </v-text-field>
            ...
            <div class="ExpansionInputContent mb-7">
              <v-text-field
                ...
                type="email"
                :error-messages="
                  requiredEmailVuelidateParent('email', 'createContactData')
                "
                label="Email"
              >
              </v-text-field>
            ...
            <div class="ExpansionInputContent mb-7">
              <v-text-field
                ...
                required
                :error-messages="
                  requiredInputVuelidateParent('phone', 'createContactData')
                "
                label="Phone Number"
              >
              </v-text-field>
            ...
            <div class="ExpansionInputContent mb-7">
              <v-text-field
                ...
                required
                :error-messages="
                  requiredInputVuelidateParent('address', 'createContactData')
                "
                label="Address"
              >
              </v-text-field>
            ...
            <div class="ExpansionInputContent mb-7">
              <v-select
                ...
                required
                :error-messages="
                  requiredInputVuelidateParent('type', 'createContactData')
                "
                label="Contact Type"
                :items="typeOptions"
                item-text="text"
            ...
            <div class="ExpansionInputContent">
              <v-switch
                v-model="createContactData.active"
                label="Active"
                color="primary"
              ></v-switch>
            </div>
      ...
      this.loadContacts()
        .then(() => {
          ...
          if (contact && (contact.type || contact.brokerOrCedentId)) {
            ...
            const contactType =
              contact.type || (contact.brokerOrCedentId ? 1 : 2); // fallback logic
  ...
  methods: {
    ...
    mapBrokerOrCedentData(contact) {
      if (contact.type === 1 || contact.type === "1") {
        const broker = this.availableBrokers.find(
          (b) => b.id == contact.brokerOrCedentId
        );
        if (broker) {
          this.createContactData.brokerOrCedentId = broker;
        }
      } else if (contact.type === 2 || contact.type === "2") {
        const cedent = this.availableCedents.find(
          (c) => c.id == contact.brokerOrCedentId
        );
        if (cedent) {
    ...
    async submitForm() {
      ...
      const contactInfo = {
        id:
          parseInt(this.contactId) !== 0 ? parseInt(this.contactId) : undefined,
        name,
        email,
        phone,
        address,
        type,
        brokerOrCedentId: brokerOrCedentId.id
          ? brokerOrCedentId.id.toString()
          : brokerOrCedentId.toString(),
      };
