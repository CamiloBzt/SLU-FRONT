<template>
  <v-dialog v-model="dialogFourEyes" width="600">
    <template v-slot:activator="{ on, attrs }">
      <v-btn v-if="subscriptionStatus < 3" rounded large depressed outlined class="btn four-eyes-button" color="#003D6D" v-bind="attrs" v-on="on"> Four eyes law </v-btn>
    </template>
    <v-card id="card-eye" class="pb-3">
      ...
      <div id="border-blue" class="mb-8">
        <v-card-text class="font-weight-bold text-h6 blue-text">Following the company's policies select the underwriter who will check the quotation.</v-card-text>
        <v-divider id="divisor"></v-divider>
        <v-autocomplete
          class="rounded-lg inputSearch mx-6 my-3"
          filled
          :clearable="false"
        ...
        />
        <v-btn class="d-flex mx-auto px-16 btnRequest" elevation="2" large rounded @click="addUnderwriters()">Request</v-btn>
...
export default {
  async beforeMount() {
    ...
    this.underwritersTable = await this.getNotificationsFourEyeSuscriptor(this.$store.state.subscription_id);
  },
  data() {
    return {
  ...
  methods: {
    ...mapActions(["getListUnderwriters", "getNotificationsFourEyeSuscriptor", "createNotificationFourEye", "socketReloadNotifications"]),
