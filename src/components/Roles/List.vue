<template>
  <div class="SubscriptionTable mt-5">
    ...
    <div class="TableTitle d-flex justify-space-between align-center">
      <!--TITULO-->
      <div class="title">SLU Roles</div>
      <div class="finishButtonCont mt-4 d-flex justify-end align-center">
        <v-btn rounded outlined large class="finishBtn" @click="goToCreate()">
          Create Role
        </v-btn>
      </div>
    ...
    <div class="table-wrapper">
      <table class="subscription-table">
        <colgroup>
          <col class="id" style="width: 10%" />
          <col class="role" style="width: 25%" />
          <col class="description" style="width: 55%" />
          <col class="actions" style="width: 10%" />
        </colgroup>
        <thead>
          <tr>
            <th class="id">ID</th>
        ...
        <tbody>
          <tr v-for="(item, i) in list" :key="i">
            <td @click="goToCreate(item)" class="id">
              {{ item.id ? item.id : "N/A" }}
            </td>
            <td class="role">{{ item.role ? item.role : "N/D" }}</td>
      ...
      <v-dialog v-model="dialog" max-width="500">
        <v-card>
          ...
          <v-card-actions class="d-flex justify-center">
            ...
            <v-btn rounded depressed class="btn-finish" @click="dialog = false">
              No
            </v-btn>
          </v-card-actions>
        </v-card>
