          ...
          <v-stepper-items>
            <v-stepper-content step="1" class="step-one">
              <div class="input-row w-100 d-flex flex-wrap">
                ...
                <InputDaysDiference :endorsementDate="effectiveDate" :expiryDate="movementEndDate" :key="movementEndDate + effectiveDate" />
              </div>
              <div v-if="showInfoEndorsement">
                ...
                <div class="input-row">
                  <div class="input-col">
                    <div class="input-cont">
                      <v-text-field v-model="sharePorcent" label="New share porcent 10" suffix="%" />
                    </div>
                  </div>
            ...
            <v-stepper-content step="2">
              ...
              <div class="input-row w-100 d-flex flex-wrap">
                <div class="input-col">
                  <div class="inner-title">Additional</div>
                  <div class="input-cont">
                    <v-autocomplete label="Clause" v-model="clause" :items="clauseList" disabled item-value="clause" item-text="clause" />
                  </div>
                </div>
                <div class="input-col">
...
export default {
  ...
  data() {
    return {
      ...
      movementEndDate: new Date(this.accountComplete.deductibles.expiryDate).toISOString().substr(0, 10),
      detail: 20,
      files: [
        {
          fileId: 1,
  ...
  watch: {
    e1: async function () {
      ...
      if (this.e1 === 3) {
        ...
        const resultOriginalCurenncy = await netPremiumInclusionRiskAutoCalcs(tivMovement, this.accountComplete.deductibles, this.sharePorcent, false, dates);
        const resultUSD = await netPremiumInclusionRiskAutoCalcs(tivMovement, this.accountComplete.deductibles, this.sharePorcent, true, dates);
